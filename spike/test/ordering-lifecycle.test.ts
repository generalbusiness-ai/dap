...
test('public proof disclosure has a finite named rule and keeps private source kinds excluded', () => {
  ...
  assert.equal(publicOpeningRule.type, 'dap.fixture.scope-public-openings/3');
  ...
  assert.deepEqual(publicOpeningRule.ineffectiveActorOnly.indeterminateReasons, ['model_unavailable', 'not_in_v1', 'package_unavailable', 'scope_runtime_required', 'unhandled']);
  assert.deepEqual(publicOpeningRule.ineffectiveActorOnly.indeterminateReasonPrefixes, ['audience_error:', 'fold_error:']);
  assert.equal(PUBLIC_OPENING_RULE_ID, 'sha256:9e9bcbdd74fe244fb63c5e339256ab508b2b3251d2e8fa642b3309cd1f1049e6');
  assert.deepEqual(publicOpeningRule.unboundActorOnly, { body: 'hidden', application: true, binding: 'absent-before-entry', known: false, authorized: false, effective: false, reason: 'unhandled', perModel: 'absent' });

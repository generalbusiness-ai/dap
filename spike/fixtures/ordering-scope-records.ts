...
export function recordScope(name: string, observation: unknown): void {
  ...
  writeFileSync(join(root,name + '.json'),canonicalize({ implementation:scopeImplementationId(),publicProofRule:PUBLIC_PROOF_RULE_ID,packages:Object.keys(packages).sort(),observation }) + '\n');
}

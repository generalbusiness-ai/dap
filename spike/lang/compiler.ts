...
const witnessFor = (table: string, code: string) => table === 'offers' ? 'corpus/sale/run1/seed-158.json' : code === 'audience from payload' ? 'corpus/sale/run6/seed-172.json' : code === 'self-asserted reference' ? 'lang/test/witnesses.test.ts#V3-F2' : table === 'applications' ? 'test/club-a5.test.ts' : undefined;
...
function expressions(expr: Expr): Record<string, unknown>[] {
  ...
  const out = typeof expr.op === 'string' ? [expr] : [];
  ...
  for (const act of d.acts) {
    ...
    if (act.readers !== 'members' && act.readers !== 'spine') {
      ...
      for (const expression of policy.recipients) {
        ...
        for (const node of expressions(expression)) if (node.op === 'path' && String(node.value).startsWith('payload.')) {
          const source = String(node.value).slice(8);
          if (act.schema[source] !== 'recipient') error('audience from payload', act.kind, 'audience', String(node.value), source, act.readers, 'referenced readers must come from preceding effective state, including on refused attempts');
        }
      }
    }
  }

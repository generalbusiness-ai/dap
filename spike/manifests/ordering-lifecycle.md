...
This O1 manifest fixes the expected observations for O4 before its transfer
implementation exists. Its sources are the spike plan §4.6 and the ordering
...
F starts at 0 with dormant rights and copied initial facts. Activation is
its first entry after genesis (and, in the origin variant, adopted
origins). Both proofs must establish effective releases to the same F and
...
Kim, F and I gain no access through the transfer. Export and proof
disclosures must stay within that budget. Public fact references and
headers may travel; the payload at S16 may not. O4 must inspect actual
...
- **Changed genesis.** `changedGenesisCases` enumerates replacement and
  ...
  candidates may fail earlier at decoding; no changed candidate may
  activate. Valid changed candidates fail with `destination_mismatch`.

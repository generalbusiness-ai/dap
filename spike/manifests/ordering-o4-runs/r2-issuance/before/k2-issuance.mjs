...
// Part A
{
  ...
  remaining = 1; skip = 0;
  let faulty;
  try { faulty = ScopeJournal.open({ backend: reopen(), writerKey: keys.W0, packages }); console.log('A: faulty open returned without throwing; fault consumed =', remaining === 0); }
  catch (e) { console.log('A: faulty open threw', e === FAULT ? 'original fault' : String(e)); }
  remaining = 0;
  if (faulty) {

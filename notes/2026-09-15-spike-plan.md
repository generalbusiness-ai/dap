...
| Disclosure completeness | Checked by the recipient's interpreter, from evidence the event carries: an application intent records `expected_activation`, the position of the attach (or genesis) that produced the binding it expects. A disclosed event whose activation position is hidden in the recipient's view pauses with `dependency_missing`; one whose activation is visible is judged against the binding active at its position, stale or effective | Design §8 leaves the checking party open. The recipient needs nothing from the discloser or the serving party, and a binding identity alone cannot say which attach activated it, since the same binding can be restored by a later attach | That the discloser or the serving party never checks; the production choice stays open |

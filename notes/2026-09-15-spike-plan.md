...
| Disclosure completeness | Checked by the recipient's interpreter, from evidence the sequencer puts in the header: `activation`, the position of the attach (or genesis) that produced the binding the event was judged under at its own position. A disclosed event whose header activation is hidden in the recipient's view pauses with `dependency_missing`; one whose activation is visible is judged by the same binding the sequencer used, stale or effective | Design §8 leaves the checking party open. The recipient needs nothing from the discloser, and only public metadata from the serving party; an intent's own provenance cannot serve, since a hidden attach between composing and sequencing changes the verdict | That the discloser or the serving party never checks; the production choice stays open |
